...
var detectedAngle = 0; // アヒルを検知した方向
...
while (true) {
  ...
  while (scanAngle < endAngle) { // スキャン
    ...
    if (angle < 70) { // アヒルの存在を検知
      ...
      oldAngleD = angle;
      isDetected = true;
      if (angle < 17 || health() < 15) {
        swim(scanAngle - 120, 100);
        var counter = 0;
  ...
  scanAndCannon(oldAngle + 72);
  scanAndCannon(oldAngle + 144);
  scanAndCannon(oldAngle + 216);
  scanAndCannon(oldAngle + 288);
